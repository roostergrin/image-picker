...
1. Create isolated worktree for spec
2. Work in dedicated environment
3. Complete development in isolation
4. Merge when ready
...
- For parallel work: suggest worktree creation
...
- Spec: .agent-os/specs/2025-01-29-user-dashboard/
- Base: main
- Allow concurrent work on auth system
```
...
- `./setup/worktree-manager.sh create <spec-path>` (verify spec exists)
- `./setup/worktree-manager.sh cleanup <feature>` (check for uncommitted changes)
...
- `git rebase`
- `git cherry-pick`
- `git worktree remove --force` (data loss risk)
